...
class MockStore {
    ...
    seed() {
        ...
        const cardIds = [-12, -6, -2].map((offset) => this.addCase({
    ...
    student(id) {
        ...
        if (!found)
            throw new Error(`查無學生 ${id}`);
        return found;
    }
    studentByNo(studentNo) {
    ...
    upsertRestriction(student, date, reason, note) {
    ...
    liftRestriction(studentId, date, reason) {
    ...
    countWindow(studentId, asOf) {
        const windowStart = shift(asOf, -(WINDOW_DAYS - 1));
        return this.cards.filter((card) => card.studentId === studentId &&
            ...
            card.countOn <= asOf).length;
    }
    progress(studentId, asOf = this.today) {
    ...
    evaluate(studentId, asOf, triggerCardId) {
        const windowStart = shift(asOf, -(WINDOW_DAYS - 1));
        const counted = this.cards.filter((card) => card.studentId === studentId &&
            card.countsTowardRecidivism &&
            card.consumedByAlertId === null &&
            ['PENDING_TEACHER', 'PENDING_OFFICE', 'COMPLETED'].includes(card.status) &&
            card.countOn >= windowStart &&
...
const delay = (value, ms = 140) => new Promise((resolve) => setTimeout(() => resolve(value), ms));
export const mockApi = {
    ...
    dashboard: () => {
        ...
        const assignments = store.assignments.filter((a) => a.status !== 'CLOSED' && a.status !== 'CANCELLED');
    ...
    teacherQueue: (classIds) => delay(store.cards
        .filter((c) => c.status === 'PENDING_TEACHER')
        .filter((c) => !classIds || classIds.length === 0 || classIds.includes(store.student(c.studentId).classId))
        .map(summary)),
    studentCards: (studentId) => delay(store.cards.filter((c) => c.studentId === studentId).map(summary)),
    card: (cardId) => {
        const card = store.cards.find((c) => c.id === cardId);
        if (!card)
            return Promise.reject(new Error('查無此反思卡'));
        return delay(toDetail(card));
    },
    alerts: () => delay([...store.alerts]),
    assignments: () => delay([...store.assignments]),
    restrictions: (date) => delay(store.restrictions.filter((r) => r.date === date)),
    searchStudent: (query) => {
        const q = query.trim();
        return delay(STUDENTS.filter((s) => s.studentNo.includes(q) || s.name.includes(q)).map((s) => ({
            id: s.id,
    ...
    student: (studentId) => {
        ...
        return delay({
            ...
            timeline,
            restrictedToday: store.restrictions.some((r) => r.studentId === student.id && r.date === store.today && r.status === 'ACTIVE'),
        });
    },
    createInfraction: async (input) => {
        const student = store.studentByNo(input.studentNo);
        if (!student)
            throw new Error(`查無學號 ${input.studentNo} 的學生`);
        const type = TYPES.find((t) => t.code === input.typeCode);
        if (!type)
            throw new Error('請選擇違規類型');
    ...
    submitCard: async (cardId, answers) => {
        const card = store.cards.find((c) => c.id === cardId);
        if (!card)
    ...
    logObserverPeriod: async (input) => {
        ...
        if (!assignment)
            throw new Error('查無派單');
        const log = assignment.periodLogs.find((p) => p.periodNo === input.periodNo);
        if (!log)
            throw new Error('查無該節次');
        if (input.action === 'CHECK_IN') {
            if (log.checkInAt)
                throw new Error(`第 ${input.periodNo} 節已報到`);
            log.checkInAt = nowIso();
            assignment.status = 'IN_PROGRESS';
        }
        else {
            if (!log.checkInAt)
                throw new Error(`第 ${input.periodNo} 節尚未報到`);
    ...
    rescheduleDuty: async (assignmentId, dutyOn) => {
