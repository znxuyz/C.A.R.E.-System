...
import { FirstOffenders } from "./pages/FirstOffenders.tsx";
...
const TITLES: Array<[RegExp, string, string?]> = [
  ...
  [
    /^\/admin\/first-offenders/,
    "初犯名單",
    "回溯期間內只有 1 次紀錄，處分前的關懷對象",
  ],
  [/^\/admin\/alerts/, "再犯警示與追蹤清單", "15 天內 3 次自動觸發"],
  [
    /^\/admin\/observers/,
    "安全觀察員值勤台",
    "一日下課 5 節・紙本檢討書回收後隔日解鎖",
  ],
...
function AdminLayout({
  ...
  useEffect(() => {
    void api
      ...
      .then((data) =>
        setCounts({
          papers: data.kpis.pendingPapers,
              ...
              !session ? (
                ...
                <AdminLayout session={session} onSignOut={signOut}>
                  <Routes>
                    ...
                    <Route
                      path="/first-offenders"
                      element={<FirstOffenders />}
                    />
