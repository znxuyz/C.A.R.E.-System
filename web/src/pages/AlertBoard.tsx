...
import { ALERT_STATUS_LABEL, ALERT_STATUS_TONE, formatDate, formatDateTime } from '../lib/format.ts';
...
export function AlertBoard() {
  ...
  const dismiss = async (alert: AlertRow) => {
    const reason = window.prompt(
      `撤銷 ${alert.studentName} 的再犯警示，請說明理由（將寫入稽核軌跡）：`,
      '',
    );
    if (!reason?.trim()) return;
    try {
      await api.dismissAlert(alert.id, reason.trim());
      toast.push('已撤銷警示，計入的違規回復為可計數狀態');
      load();
    } catch (error) {
  ...
  return (
    <>
      <Callout tone="warning">
        ...
        <span>
          觸發規則：同一學生於 <strong>15 天內（含當天）</strong>累計
          <strong> 3 次</strong>違規（不論走廊奔跑或口出穢言）即發出警示，並自動排入
          安全觀察員追蹤清單。已計入的違規會被「認列」，不會對同一波處分重複觸發。
        </span>
      </Callout>

      <Panel title="再犯警示" hint={rows ? `${rows.length} 筆` : '載入中…'} flush>
        {!rows ? (
          <div className="card__body muted">載入中…</div>
        ) : rows.length === 0 ? (
          <EmptyState title="目前沒有再犯警示" hint="持續關注 15 天內已 2 次的學生。" />
        ) : (
                  ...
                  <Fragment key={alert.id}>
                    <tr>
                      <td className="cell-strong">
                        <Link to={`/admin/students/${alert.studentId}`}>
                      ...
                      <td>
                        <Badge tone={ALERT_STATUS_TONE[alert.status]} dot>
                      ...
                      <td>
                        <div className="btn-row">
                          ...
                          {alert.assignmentId && alert.status !== 'DISMISSED' && alert.status !== 'CLOSED' && (
                    ...
                    {expanded === alert.id && (
                      <tr>
                        <td colSpan={7} style={{ background: 'var(--surface-sunken)' }}>
                          <div className="small">
                            <strong>計入本次警示的違規</strong>（已認列，不再重複計數）
                          </div>
                          <ul className="small" style={{ margin: '6px 0 0', paddingLeft: 18 }}>
                            {alert.breakdown.map((item) => (
                              <li key={item.infractionId}>
                                {formatDate(item.occurredOn)}｜{item.typeName}
                              </li>
                            ))}
                          </ul>
                        </td>
