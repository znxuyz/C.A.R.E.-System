...
```
找學生（兩條路，擇一即可）
   ① 學號或姓名：子字串比對，打「小明」「六年四班」都找得到
       └→ 多位相符時列出候選按鈕（同名同姓一律要自己點，避免登錯人）
   ② 班級 → 座號：兩個下拉，知道座號但不確定姓名時用
   └→ 選定後顯示「姓名｜班級 座號｜N 天內已 M 次」，可按「換一位」重選
違規類型（大按鈕，各自標示會發哪張紙本卡；類型與卡名於系統設定自訂）
   🏃 走廊奔跑   👉 校園安全反思卡
   💬 口出穢言   👉 口說好話反思卡
地點（熱點優先） + 節次 + 補充說明（選填）
   └→ [登錄違規]
...
### 3.5 初犯名單 `/admin/first-offenders`

回溯期間內**只有 1 筆**、且尚未被任何警示認列的學生。
依預設政策這一次只做記錄勸導（未發反思卡），因此這份清單的定位是
**處分還沒發生前的介入點**：導師晤談、個別關懷、必要時通知家長。

| 欄位 | 說明 |
|---|---|
| 學生 | 班級＋姓名（可點入歷程）、座號與學號 |
| 違規日／類型／地點・節次 | 該筆紀錄的內容 |
| 處理 | 「記錄勸導」或「已發反思卡」（視 `cardFromOffense` 設定） |
| 紀錄到期 | 這筆違規退出回溯期的日期與剩餘天數 |

「紀錄到期」是刻意放上來的：它讓「**表現穩定就會歸零**」變成可以對學生說明的具體日期，
而不是模糊的承諾。到期後該生自動離開名單，不需人工清除。

### 3.6 再犯警示 `/admin/alerts`
...
| 區塊 | 可調項目 |
|---|---|
| 再犯偵測 | 回溯天數（1–365，預設 15）、觸發次數（1–20，預設 3）、**發卡起算次數**（1–20，預設 2；第一次只記錄勸導） |
| 違規類型與反思卡 | 以圖卡呈現，可新增／編輯／刪除；每類可自訂**要寫哪一張反思卡**（卡名自由命名）、統計分類與是否計入再犯 |
| 地點 | 以圖卡呈現，可新增／刪除／切換熱點 |
| 學生名冊 | 上傳 Excel（.xlsx）或 CSV，亦可直接貼上；匯入前預覽新增／重新編班／停用人數與問題列 |
| 名冊換屆 | 「完整名冊」模式：不在檔案中的在校生標記為畢業／轉出並停用（保留歷程，不可再登錄） |
