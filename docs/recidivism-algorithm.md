...
`recidivismWindowDays` 允許 **1–365 天**（安全規則與前端同時驗證），
後台提供 15 天 / 一個月 / 一學期（120）/ 一學年（365）四個快捷鍵，
也可直接輸入任意天數。設定長視窗前請注意三件事：

1. **判定會變嚴。** 同樣是「3 次」，在 365 天內累計與在 15 天內累計，
   是完全不同的管教政策；長視窗較接近「學年累計制」，
   建議同時把 `recidivismThreshold` 往上調（例如 365 天 / 6 次）。
2. **快取陣列會變長。** `students/{id}.recidivismWindow` 只保留
   「視窗內、尚未被警示認列」的違規；一次警示會認列並清掉 3 筆，
   因此即使設一整年，單一學生的陣列通常仍只有個位數，
   遠低於 Firestore 單份文件 1 MiB 的上限。
3. **調整只對之後登錄的違規生效。** 視窗是登錄當下即時計算的，
   既有警示不會回溯重算（快照設計使然）。
   若要讓新參數立刻反映在既有資料上，需重新登錄或手動處理。
