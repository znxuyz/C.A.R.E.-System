...
sequenceDiagram
  ...
  O->>SYS: 登錄違規（學號或姓名、類型、地點、節次）
  SYS->>SYS: 回溯 15 天判定這是期間內第幾次
  alt 期間內第 1 次（未達發卡次數）
    SYS-->>O: 顯示「僅記錄勸導，不發卡、下課不受影響」
    O->>S: 口頭勸導
  else 第 2 次起
    SYS->>SYS: 凍結當日自由下課
...
> **發卡起算次數（`settings.cardFromOffense`，預設 2）**
> 期間內第一次以口頭勸導與紀錄為主，**學生不必寫任何東西、下課照常**；
> 累積到第 2 次才發紙本反思卡並凍結當日自由下課。
> 兩種情形都會計入再犯次數。設為 1 即回到「每次都發卡」。
> 未發卡的紀錄直接以 `DONE` 結案（沒有待回收的紙本）；
> 安全規則允許以 `OPEN` 或 `DONE` 建立，但 `DONE` 必須伴隨 `cardIssued: false`，
> 避免發了卡卻略過回收流程。
>
> 這些只記錄勸導的學生會出現在 **追蹤輔導 → 初犯名單**
> （`/admin/first-offenders`），並標示紀錄何時退出回溯期，
> 供導師在處分發生前晤談介入。
