...
1. 按一次 **「建立內建違規類型與地點」**
   ...
   > 按之前登錄畫面仍可使用：類型與地點會先用程式內建的預設值。

2. **「違規類型與反思卡」** 以圖卡列出所有類型，可直接維護：
   - **編輯／✕**：修改或刪除既有類型
   - **＋ 新增違規類型**：可設定
     | 欄位 | 說明 |
     |---|---|
     | 圖示 | 登錄畫面大按鈕上的 emoji |
     | 違規類型名稱 | 例：走廊奔跑、亂丟垃圾 |
     | **要寫哪一張反思卡** | 卡名可自由命名（例：愛整潔反思卡）；登錄後畫面會提示發這張卡 |
     | 統計分類 | 安全類／言語類，只影響儀表板趨勢圖的分色 |
     | 計入再犯次數 | 取消勾選的類型仍會登錄發卡，但不累計到再犯門檻（勸導性質用） |

   > 既有違規紀錄已存下登錄當時的類型名稱與卡名，改名或刪除都不影響歷史資料。

3. **「地點」** 同樣以圖卡列出，可直接維護：
   - **✕** 刪除地點（既有違規紀錄已存下當時的地點名稱，不受影響）
   - **🔥 熱點／設為熱點** 切換是否列為熱點（熱點會在儀表板的地點統計優先呈現）
   - **新增地點**：輸入名稱（可勾「列為熱點」）→ 按「新增」

   > 第一次增刪時，系統會先把內建預設值寫進 Firestore 再套用你的異動，
   > 否則刪掉一個之後預設值又會整組出現。
...
「學生名冊 → 從 Excel／CSV 匯入」選擇校務系統匯出的 `.xlsx` 或 `.csv`：

- 欄位需包含 **班級／座號／學號／姓名**，標題列順序不拘
  （也認得「班級名稱／座位號碼／學生證號／學生姓名」等常見寫法）
- 沒有標題列時，依 `班級, 座號, 學號, 姓名` 的順序讀取
- 自動取第一個有資料的工作表，空白列會略過
- CSV 若是 Windows 版 Excel 另存的 Big5 編碼也能正確讀取（不會變亂碼）
- 舊版 `.xls` 不支援，請先在 Excel 另存為 `.xlsx`
...
| 模式 | 行為 | 什麼時候用 |
|---|---|---|
| **新增／更新**（預設） | 只處理檔案中的學生：沒有的新增、已有的更新班級與座號。檔案中沒有的人完全不動。 | 學期中轉入、個別修正 |
| **完整名冊（新學年換屆）** | 把檔案當成「本學年在校生的完整名冊」：不在檔案中的在校生會標記為**已畢業／轉出並停用**。 | 每學年開學重新編班 |

兩種模式都會先**預覽**：解析到幾位學生、幾個班級，比對現有名冊後會新增幾位、
重新編班幾位、（完整名冊模式）要停用幾位，以及哪幾列有問題
（欄位不足、學號重複、座號非整數）。確認後再按「匯入 N 位學生」；
要停用學生時還會再跳一次確認，並列出前幾位的名字讓你核對。
...
學號是唯一不變的鍵，因此換屆只要**匯入一份新的完整名冊**就好：

| 情況 | 系統怎麼處理 |
|---|---|
| **重新編班**（學號不變、班級座號變） | 視為更新：班級／座號改掉，違規歷程、再犯紀錄全部跟著學生走 |
| **新生／轉入**（新學號） | 自動新增 |
| **畢業／轉出**（學號不在新名冊中） | 標記 `active: false` 並記錄 `leftOn`，**不刪除** |
| **休學復學、轉出又轉回** | 學號再次出現在名冊中即自動重新啟用，沿用原本的歷程 |

停用（而非刪除）的意義：

- 違規紀錄、再犯警示、稽核軌跡全部保留，日後查詢或申訴都查得到
- 但**不能再登錄新違規**：登錄畫面輸入該學號會顯示「已畢業／轉出」並擋下送出
- 需要重新啟用時，把他放回名冊再匯入一次即可

> 年度作業建議：開學時先用「完整名冊」匯入全校在校生，之後學期中的轉入轉出
> 用「新增／更新」處理即可。
